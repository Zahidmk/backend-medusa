...
/**
 * Core brand sync logic, shared by the scheduled job and the
 * admin "Sync Now" route (POST /admin/odoo/sync-brands) so both
 * stay in lockstep instead of drifting like the category sync
 * paths did.
 */
export async function runBrandSync(container: MedusaContainer, logger: any): Promise<BrandSyncResult> {
  ...
  try {
    ...
    try { odooSyncService = container.resolve("odooSyncService"); } catch {
      ...
      return { total: 0, created: 0, updated: 0, errors: 0, error_details: ["OdooSyncService not registered"] };
    }
  }
  ...
  try {
    ...
    return { total: 0, created: 0, updated: 0, errors: 0, error_details: ["BRAND_MODULE not registered"] };
  }
  ...
  logger.info(`[Brand Sync] Completed: ${created} created, ${updated} updated, ${errors} errors.`);
  ...
  try {
    ...
    await pgConnection.raw(
      `INSERT INTO system_config (key, value, updated_at)
       VALUES ('odoo_last_brand_sync', ?, NOW())
       ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()`,
      [new Date().toISOString()]
    );
  } catch { /* ignore */ }

  return { total: brands.length, created, updated, errors, error_details: errorDetails };
}
...
export default async function odooBrandSyncJob(containerOrObj: any) {
  ...
  try {
    await runBrandSync(container, logger);
  } catch (error: any) {
